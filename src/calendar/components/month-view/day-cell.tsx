...
export function DayCell({ cell, events, eventPositions }: IProps) {
  const { setViewCalendar, setSelectedDate } = useCalendar();
  ...
  return (
    <DroppableDayCell cell={cell}>
      <div
        className={cn(
          "flex h-full flex-col gap-1 border-r border-t border-gray-200 dark:border-gray-800 py-1.5 lg:py-2",
          isSunday && "border-r-0 border-gray-200 dark:border-gray-800"
        )}
      >
        <span
          onClick={() => goToDay()}
          className={cn(
            "h-6 px-1 text-xs font-semibold lg:px-2 cursor-pointer",
        ...
        {cellEvents.length > MAX_VISIBLE_EVENTS && (
          ...
          >
            ...
            <span className="hidden sm:inline">
              {" "}
              {cellEvents.length - MAX_VISIBLE_EVENTS} più...
            </span>
          </p>
        )}
      </div>
